package AlgorithmVisualizer.Swing.LogTable;
...
public class TableScrollBarUI extends BasicScrollBarUI {
    ...
    protected void paintThumb(Graphics grphcs, JComponent jc, Rectangle rctngl) {
        ...
        if (isDragging) {
            g2.setColor(new Color(0, 158, 113));
        } else {
            if (isThumbRollover()) {
                g2.setColor(new Color(0, 177, 127));
            } else {
        ...
        if (scrollbar.getOrientation() == JScrollBar.VERTICAL) {
            g2.fill(new RoundRectangle2D.Double(rctngl.getX() + space, rctngl.getY() + space, rctngl.getWidth() - space * 2, rctngl.getHeight() - space * 2, round, round));
        } else {
            g2.fill(new RoundRectangle2D.Double(rctngl.getX() + space, rctngl.getY() + space, rctngl.getWidth() - space * 2, rctngl.getHeight() - space * 2, round, round));
        }
        g2.dispose();
    ...
    private class ScrollButton extends JButton {
        ...
        public void paint(Graphics grphcs) {
            ...
            int x = (width - 5) / 2;
            if ((increase && isMax) || (!increase && isMin)) {
                g2.setColor(new Color(153, 232, 209));
            } else {
                if (pressed) {
